...
async function sharedHaikuLogic(formData, user) {
  ...
  if(!isAlphaNumericWithBasics(ourHaiku.line1)) errors.line1 = "no Special charachters allowed"
  if(!isAlphaNumericWithBasics(ourHaiku.line2)) errors.line2 = "no Special charachters allowed"
  if(!isAlphaNumericWithBasics(ourHaiku.line3)) errors.line3 = "no Special charachters allowed"
...
export const createHaiku = async function (prevState, formData) {
  ...
  const result = await sharedHaikuLogic(formData, user);
  ...
  if(result.errors.line1 || result.errors.line2 || result.errors.line3){
    ...
    const newHaiku = await haikuCollection.insertOne(result.ourHaiku)
    return redirect("/")
...
export const editHaiku = async function (prevState, formData) {
  ...
  if(result.errors.line1 || result.errors.line2 || result.errors.line3){
    ...
    await haikusCollection.findOneAndUpdate({ _id: ObjectId.createFromHexString(haikuId)}, {$set: result.ourHaiku})
    return redirect("/")
};
